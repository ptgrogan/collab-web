$(function() {
  ...
  var update_task = function(data) {
    ...
    for(i = 0; i < task.inputs.length; i++) {
      x_i = task.inputs[i];
      $(".row-inputs").append('<div class="col-2"><div class="card"><div class="card-header text-center"><label for="x"'+(x_i+1)+'>X<sub>'+(x_i+1)+'</sub></label></div><div class="card-body"><div id="x'+(x_i+1)+'" class="mx-auto" style="height:300px;"></div></div></div></div>');
      inputs[i] = $('#x'+(x_i+1))[0];
